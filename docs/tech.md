...
- Debug build: bundle ID `com.monawwar.Jelly.Debug`, display name "Jelly (Debug)", so it never collides with an installed release. It reads and writes `~/.config/jelly/jelly-debug.toml` instead of `jelly.toml`, so settings a release doesn't know yet never show up there as config problems. Themes are shared.
...
Jelly/                         App target (file-system synchronized group)
  ...
  Features/
    ...
    SessionSwitcher/           SessionSwitcherModel, SessionSwitcherView, SessionSwitcherTile (⌃⇥ overlay)
    ...
    QuickTerminal/             QuickTerminalController (panel, hotkey, keys), GlobalHotKey (Carbon), QuickTerminalModel (one shell surface), QuickTerminalOutput, views
  ...
  Shared/                      Metrics, IconButton, AppInfo, KeyChord+Event, KeyChord+Carbon, ThemeColor+SwiftUI, FileWatcher
...
Packages/
  JellyCore/                   No UI. Plain Sendable value types.
    Sources/JellyCore/
      ...
      Activity/                ProcessRecord, ProcessTree (descendants of a shell), ActivitySnapshot and its parts, ProcessIdentity+Summary
  ...
  JellyTerminal/               Everything that touches SwiftTerm
    Sources/JellyTerminal/
      ...
      Activity/                ActivitySampler (actor), ProcessTable (libproc), SocketTable (listening TCP sockets), SystemCounters (host CPU, VM, interfaces), MachTime
...
- **Activity.** A card over the pane area (Toggle Activity in the palette or View menu, `activity.toggle`) with Processes, Ports and System tabs. `ActivityModel` exists only while the card is open: every 2s it collects each pane's shell pid on main and hands them to the `ActivitySampler` actor, which does all the work off main with libproc and sysctl, no `ps` or `lsof`. One pass lists every pid (`proc_listallpids`), reads parent, terminal group, name and start time (`PROC_PIDTBSDINFO`) and CPU time and footprint (`proc_pid_rusage`, Mach ticks converted with the timebase), then walks each shell's descendants. A pane's foreground process is its shell's terminal group; commands come from `KERN_PROCARGS2`, summarised (`npm run dev`, interpreter dropped for bin shims) and cached per pid and start time. CPU % is the CPU time delta over wall time, per core like Activity Monitor; a sample less than 0.5s after the last keeps the previous rates. The busiest other processes of the user fill the Other section. Ports are scanned only while that tab is showing: `PROC_PIDLISTFDS` and `PROC_PIDFDSOCKETINFO` over the user's processes, keeping TCP sockets in `LISTEN`, one row per port, a pane owner winning over others, flagged when bound beyond loopback. System reads `HOST_CPU_LOAD_INFO`, `HOST_VM_INFO64` (used = app + wired + compressed, as Activity Monitor), `NET_RT_IFLIST2` byte counters for up, non-loopback, non-point-to-point interfaces, and the load average; the model keeps the last 60 samples for the graphs. A pass takes a few milliseconds. The tab and corner are remembered in `UserDefaults`.
...
- **Session switcher.** `session.next` / `session.previous` pressed with ⌘, ⌃ or ⌥ held open a `SessionSwitcherModel` instead of switching; without a modifier (the palette) they switch at once. Tiles are ordered by recency (`WindowModel` records each selected session), starting on the current one and moving one step, so a quick tap returns to the previous session. It keeps the held modifiers, and a `.flagsChanged` monitor on the window commits the selection as soon as one of them is released. The overlay appears only after 0.12 s, so a quick tap switches without flashing it. While it's open, the key monitor lets the session next/previous bindings through and takes Esc, ↩, ← and → itself; other keys are swallowed. Losing key window cancels it.
- **Quick terminal.** `QuickTerminalController` registers `quick-terminal.hotkey` with Carbon's `RegisterEventHotKey`, which works from any app without Accessibility permission, and re-registers it when the config changes. A registration failure becomes a runtime diagnostic. The window is a borderless, non-activating `QuickTerminalPanel` at floating level that joins the active Space and full-screen apps, so it takes the keyboard without bringing Jelly forward. It hides when it loses key, and key goes back to the Jelly window that had it. The panel is the card plus a shadow margin, placed at the bottom centre of the screen under the mouse; SwiftUI reports the card's height and the panel resizes around it, keeping its bottom edge, so clicks outside the card reach the app below. `QuickTerminalModel` owns one `TerminalSurface` running the user's shell like a pane (in-app PTY, never `jelly-mux`, so session pruning can't end it), started on first show in the home folder. The surface is sized to the panel's width and 16 rows before it starts, so the shell sees the real grid, and sits in a clipping view (`QuickTerminalOutput`) that shows only the rows in use: the surface reports `contentRows` (the last non-empty row or the cursor's row, or every row on the alternate screen) through `onContentRowsChange`, only when the number changes. That callback and the directory are the only things that reach SwiftUI. The badge reads the shell's directory with `proc_pidinfo` once a second while the panel is open, and on OSC 7. ⌘↩ detaches the surface (clearing its callbacks) and hands it to `WorkspaceModel.adoptTab`, which wraps it in a `PaneModel` and a new tab, so the same process carries on in the main window. Keys go to the surface; a local monitor on the panel only takes ⌘ shortcuts (⌘↩, ⌘K, ⌘W, copy, paste, select all; others are swallowed so they don't act on a window behind, except ⌘Q and ⌘,) and Esc when the shell has no foreground process.
...
- When a check finds a version, Sparkle stays quiet on scheduled checks and an "Update available" button appears before the find button in the tab bar; clicking it opens Sparkle's update window.
...
- `ProcessTree` / `ProcessIdentity.summary`: descendants depth-first with depths, cycles cut; commands summarised with login dashes, interpreters in front of bin shims and paths dropped.
